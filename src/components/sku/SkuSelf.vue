<template>
  <div class="sku">
    <div
      class="spec_item"
      v-for="a in format_spec"
      :key="a.format_spec_item_key"
    >
      <div class="spec_item_key">{{ a.format_spec_item_key }}</div>
      <div class="spec_item_value_all">
        <!-- 点击红色的时候给select_spec['颜色']添加'红色' -->
        <!-- 即select_spec['颜色']=='红色' -->
        <!-- 因此当遍历到b.format_spec_item_value = 绿色 -->
        <!-- select_spec['颜色'] !== 绿色 -->
        <!-- 因此不添加active类名 -->
        <div
          :class="[
            ...
            {
              active: isActive(
                a.format_spec_item_key,
                b.format_spec_item_value
              ),
            },
          ]"
          class="spec_item_value"
          v-for="b in a.format_spec_item_value_all"
          :key="b.format_spec_item_value"
          @click="
...
<script>
import { original_spec, sku } from "utils/sku";
export default {
  name: "SkuSelf",
  data() {
    return {
      ...
      format_spec: [], //格式化后的规格键值数据,format_spec1是示例
      format_spec1: [
        {
          //a
          format_spec_item_key: "颜色",
          format_spec_item_value_all: [
            { format_spec_item_value: "红色", able: true }, //b
            { format_spec_item_value: "紫色", able: true },
            { format_spec_item_value: "白色", able: true },
            { format_spec_item_value: "黑色", able: true },
          ],
        },
        {
          format_spec_item_key: "套餐",
          format_spec_item_value_all: [
            { format_spec_item_value: "套餐一", able: true },
            { format_spec_item_value: "套餐二", able: true },
            { format_spec_item_value: "套餐三", able: true },
            { format_spec_item_value: "套餐四", able: true },
          ],
        },
        {
          format_spec_item_key: "存储",
          format_spec_item_value_all: [
            { format_spec_item_value: "64G", able: true },
            { format_spec_item_value: "128G", able: true },
            { format_spec_item_value: "256G", able: true },
          ],
        },
      ],
    };
  ...
  created() {
    ...
    this.format_spec = original_spec.map((a) => {
      return {
        format_spec_item_key: a.original_spec_item_key,
        format_spec_item_value_all: a.original_spec_item_value_all.map((b) => {
          return {
            format_spec_item_value: b,
  ...
  methods: {
    ...
    change_spec_value_status(key, value, able) {
      ...
      //修改format_spec的able
      this.format_spec.forEach((a) => {
        a.format_spec_item_value_all.forEach((b) => {
